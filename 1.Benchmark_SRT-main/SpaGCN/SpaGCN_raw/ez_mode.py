...
def detect_SVGs_ez_mode(adata, target, x_name, y_name, domain_name, min_in_group_fraction, min_in_out_group_ratio, min_fold_change):
	...
	#内外组比率(in_out_group_ratio)，它是目标聚类中表达该基因的细胞比例与邻近聚类中表达该基因的细胞比例的比值。这个比率用于识别在目标聚类中特异性表达的基因。
	#组内分数，它是目标聚类中表达该基因的细胞比例。这个值越高，表示基因在目标聚类中的表达越特异性。
	#fold_change：折叠变化值，它是目标聚类与邻近聚类之间基因表达水平的比值的对数。这个值用于衡量基因表达在不同聚类之间的变化程度。
