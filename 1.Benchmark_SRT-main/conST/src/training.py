...
class conST_training:
    ...
    def get_embedding(self):
        self.model.eval()
        self.adj_norm=self.adj_norm.to_dense() #因为返回的adj为sparse_tensor
        # print("type(self.adj_norm)",type(self.adj_norm))
    ...
    def major_training(self):
        ...
        test_z, _, _, _ = self.process() #(3798,28)
        y_pred_last = np.copy(kmeans.fit_predict(test_z)) #3798个标签

        self.model.cluster_layer.data = torch.tensor(kmeans.cluster_centers_).to(self.device) #（10，28）
        self.model.train()
        ...
        for epoch_id in range(self.epochs):
            ...
            if self.use_img is False:
                ...
                ret_l2c = self.model.l2c_forward(feat_x, feat_x3, self.beta) #1
