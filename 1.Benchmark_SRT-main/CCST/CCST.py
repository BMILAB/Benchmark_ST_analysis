...
def train_DGI(args, data_loader, in_channels):
    ...
    if args.load:
        DGI_filename = args.model_path+'DGI_lambdaI_' + str(args.lambda_I) + '_epoch' + str(args.num_epoch) + '.pth.tar'  #导入已经训练好的模型
        DGI_model.load_state_dict(torch.load(DGI_filename))
    else:  # 自己训练

        import datetime
        start_time = datetime.datetime.now()
        for epoch in range(args.num_epoch): #args.num_epoch
            DGI_model.train()
        ...
        # DGI_filename =  args.model_path+'DGI_lambdaI_' + str(args.lambda_I) + '_epoch' + str(args.num_epoch) + '.pth.tar' #把训练好的模型保存
