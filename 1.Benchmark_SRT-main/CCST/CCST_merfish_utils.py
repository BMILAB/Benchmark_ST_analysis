...
def get_data(args):
    ...
    if args.data_name=='MERFISH':
     ###当数据为MerFish时，有个batch_info
        cell_type_indeces = np.load(data_folder + 'cell_batch_info.npy')

    elif args.data_name == 'STARmap':
        print("data_folder + 'cell_types.npy':",data_folder + 'cell_types.npy')
        cell_type_indeces = np.load(data_folder + 'cell_types.npy',allow_pickle=True)
        print("cell_type",cell_type_indeces)
        ...
        if n_clusters <=5:
            ...
            for tmp in range(n_clusters):
                if tmp in cell_cluster_batch_list:
                    colors_list.append(all_colors_map[tmp]) #每种类型的标签用同一种颜色绘制
...
def CCST_on_MERFISH(args):
    ...
    if args.DGI:
        ...
        print("CCST_on_MERFISH中数据导入完成，开始训练：")
        ...
        for data in data_loader:
            ...
            np.save(X_embedding_filename, X_embedding) # (X,256)
            print("数据的嵌入保存完成！")
    ...
    if args.cluster:
        ...
        #     all_data.append([index, cell_type_indeces[index], cluster_labels[index]]) #真实的：预测的   Merfish中叫cell_batch_info，与cell_type_indeces类似
        #     np.savetxt(args.result_path + '/types.txt', np.array(all_data), delimiter='\t')
        result_df = pd.DataFrame()
        result_df["Ground Truth"] = cell_type_indeces
        result_df['CCST_Kmens_predict'] = cluster_labels
        result_df.to_csv(args.result_path + '/predict_types.csv')

        ari, nmi, ami = eval_model(cell_type_indeces, cluster_labels)
        print("********************************预测结果输出**********************：")
        # print(args.sample_name, ari, nmi, ami, SC_Score) # DLPFC
        print("ari, nmi, ami,SC_Score:", ari, nmi, ami, SC_Score)

        results_file = args.result_path + '/CCST_adata.h5ad'
