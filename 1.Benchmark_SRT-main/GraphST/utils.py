...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='emb_pca', random_seed=2020):
    ...
    """
    print("begin mclust clustering")
    start=time.time()
    np.random.seed(random_seed)
    import rpy2.robjects as rp_robjects
    rp_robjects.r.library("mclust")
   # print("type(rmclust)",type(rmclust)) # <class 'rpy2.robjects.vectors.StrVector'>
    ...
    res = rmclust11(rpy2.robjects.numpy2ri.numpy2rpy(adata.obsm[used_obsm]), num_cluster, modelNames)
    ...
    print("mclust finished：",end-start)
    return adata
...
def refine_label(adata, radius=50, key='label'):
    ...
    for i in range(n_cell):
        ...
        for j in range(1, n_neigh+1):
            neigh_type.append(old_type[index[j]]) #Getting the index of x neighbor
        max_type = max(neigh_type, key=neigh_type.count) #Get the neighbor type with the most
